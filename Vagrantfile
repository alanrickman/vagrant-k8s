...
nodes = [
  { :type => 'master', :box => BOX, :private_network => PRIVATE_NETWORK, :ram => 2048, :cpu => 2 },
  { :type => 'worker', :count => 2, :box => BOX, :private_network => PRIVATE_NETWORK, :ram => 1024, :cpu => 2 },
]
  ...
  nodes.each do |node|
    ...
    if "#{node[:type]}" == "master" then
      config.vm.define "#{node[:type]}" do |node_config|
        ...
        # Run shared provisoning script
        node_config.vm.provision "shell", path: "./scripts/provision_vm.bash"
